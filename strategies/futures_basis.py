...
CACHE_DIR = os.path.join(ROOT, "data", "futures_basis")
...
def _cached_payload(capital, lookback):
    points = _read_csv(os.path.join(CACHE_DIR, "backtest_points.csv"))
    if not points:
        return None, "Cached futures basis CSV is missing."
    ...
    if not points:
        return None, "Cached futures basis CSV has no rows for the selected lookback."
    ...
    return {
        ...
        "metrics": metrics,
        "points": scaled,
        "source": "data/futures_basis/backtest_points.csv",
    }, None
